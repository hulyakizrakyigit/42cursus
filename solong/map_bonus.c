...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/22 18:13:39 by hkizrak-          #+#    #+#             */
/*   Updated: 2023/12/22 20:03:01 by hkizrak-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_width(t_info *info, int old_width, char *line)
{
	if (!line)
		return ;
	if (old_width != ft_strlen_endl(line))
		exiterr(12, "invalid map, line lenghts are incompatible", info);
}
...
void	init_map(t_info *info, char *map_path)
{
	char	*line;
	int		fd;
	int		i;
	...
	if (close(fd) == -1)
		exiterr(12, "map file cannot be closed", info);
}
	...
	while (y < info->map->height)
	{
		...
		else
		{
			if (info->map->board[y][x] != M_WALL
				...
				exiterr(16, "invalid map border", info);
		}
		y++;
	}
}

static void	check_map(t_info *info, char *map_path)
{
	char	*file_ext;
